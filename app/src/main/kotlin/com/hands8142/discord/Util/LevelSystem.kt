...
fun appendXp(userId: Long, guildId: Long, xp: Int): Boolean {
    if (database !== null) {
        ...
        if (user.next()) {
            ...
            val updateSql = "UPDATE level SET level = $newLevel, xp = $newXp WHERE guildId = $guildId AND userId = $userId"
...
suspend fun computeLeaderboard(kord: Kord, leaderboard: List<HashMap<String, Any>>): ArrayList<HashMap<String, Any>> {
    val computedArray: ArrayList<HashMap<String, Any>> = ArrayList()

    leaderboard.map {
        val row = HashMap<String, Any>()
        val username = if (kord.getUser(Snowflake(it["userId"].toString().toLong()))?.username !== null) kord.getUser(Snowflake(
            it["userId"].toString().toLong()))?.username!! else "Unknown"
        val discriminator = if (kord.getUser(Snowflake(it["userId"].toString().toLong()))?.username !== null) kord.getUser(Snowflake(
            it["userId"].toString().toLong()))?.discriminator!! else "0000"
